...
LOGS_PATH = os.path.join(ROOT_PATH, 'log', 'nested_unet_glomerulus')
...
transform = transforms.Compose([transforms.Resize((256, 256)), transforms.ToTensor(),
                                transforms.Normalize(mean=glomerulus.MEAN, std=glomerulus.STD)])
...
# 初始化模型、优化方法、损失函数
# TODO: 记得修改模型
net = NestedUNet(num_classes=NUM_CLASSES, in_channels=glomerulus.CHANNELS_NUM,
...
for epoch in range(args.epoch):
    ...
    if (epoch + 1) % args.save_frequency == 0:
        ...
        checkpoint_path = os.path.join(CHECKPOINT_DIR_PATH,
                                       'nested_unet_glomerulus_{}.pkl'.format(
