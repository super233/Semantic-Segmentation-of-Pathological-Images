...
from models.loss_function import LovaszLoss
from models.nested_unet import NestedUNet
from datasets import ultrasound_nerve
...
LOGS_PATH = os.path.join(ROOT_PATH, 'log', 'nested_unet_ultrasound_nerve')
...
# GAMMA = args.gamma
NUM_CLASSES = ultrasound_nerve.NUM_CLASSES
...
transform = transforms.Compose([transforms.Resize((256, 256)), transforms.ToTensor(),
                                transforms.Normalize(mean=ultrasound_nerve.MEAN, std=ultrasound_nerve.STD)])
target_transform = transforms.Compose([transforms.Resize((256, 256)), transforms.ToTensor()])

train_data = ultrasound_nerve.UltrasoundNerve(mode='train', transform=transform, target_transform=target_transform)
val_data = ultrasound_nerve.UltrasoundNerve(mode='val', transform=transform, target_transform=target_transform)
...
# 初始化模型、优化方法、损失函数
# TODO: 记得修改模型
net = NestedUNet(num_classes=NUM_CLASSES, is_deep_supervision=True, is_dense_connections=True, l_num=4)
...
for epoch in range(args.epoch):
    ...
    if (epoch + 1) % args.save_frequency == 0:
        ...
        checkpoint_path = os.path.join(CHECKPOINT_DIR_PATH,
                                       'nested_unet_ultrasound_nerve_{}.pkl'.format(
