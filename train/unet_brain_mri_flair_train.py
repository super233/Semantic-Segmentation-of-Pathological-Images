...
from datasets import brain_mri
...
LOGS_PATH = os.path.join(ROOT_PATH, 'log', 'unet_brain_mri_flair')
...
NUM_CLASSES = brain_mri.NUM_CLASSES
...
transform = transforms.Compose([transforms.ToTensor(),
                                transforms.Normalize(mean=brain_mri.MEAN, std=brain_mri.STD)])
target_transform = transforms.Compose([transforms.ToTensor()])

train_data = brain_mri.BrainMri(mode='train', transform=transform, target_transform=target_transform)
val_data = brain_mri.BrainMri(mode='val', transform=transform, target_transform=target_transform)
...
for epoch in range(args.epoch):
    ...
    if (epoch + 1) % args.save_frequency == 0:
        ...
        checkpoint_path = os.path.join(CHECKPOINT_DIR_PATH,
                                       'unet_brain_mri_flair_{}.pkl'.format(
