...
from models.unet_resnet34 import UNet_ResNet34
...
LOGS_PATH = os.path.join(ROOT_PATH, 'log', 'unet_resnet34_voc')
...
print('<================== Parameters ==================>')
print('model: UNet based on ResNet34.')
...
# 原始数据以及通过padding和resize修改为256*256的了，目前不进行其他转换
# 这里的mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]，貌似是一个自然图像的经验值
transform = transforms.Compose([transforms.ToTensor(),
                                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
target_transform = transforms.Compose([transforms.ToTensor()])

train_data = voc.VOC(mode='train', transform=transform, target_transform=target_transform)
val_data = voc.VOC(mode='val', transform=transform, target_transform=target_transform)
...
for epoch in range(args.epoch):
    ...
    if (epoch + 1) % args.save_frequency == 0:
        ...
        checkpoint_path = os.path.join(CHECKPOINT_DIR_PATH,
                                       'unet_resnet34_{}.pkl'.format(time.strftime('%m%d_%H%M', time.localtime())))
        torch.save({
