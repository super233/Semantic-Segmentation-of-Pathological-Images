...
LOGS_PATH = os.path.join(ROOT_PATH, 'log', 'unet_logs')

# 创建Tensorboard可视化的log目录，若存在，则删除再创建
if os.path.exists(LOGS_PATH):
    shutil.rmtree(LOGS_PATH)
os.mkdir(LOGS_PATH)
writer = SummaryWriter(log_dir=LOGS_PATH, comment='unet', flush_secs=60)
...
for epoch in range(args.epoch):
    ...
    if (epoch + 1) % args.save_frequency == 0:
        checkpoint_path = os.path.join(CHECKPOINT_DIR_PATH,
                                       'unet_{}.pkl'.format(time.strftime('%m%d_%H%M', time.localtime())))
        torch.save({
